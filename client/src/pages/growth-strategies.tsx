...
export default function GrowthStrategies() {
  ...
  const handleStartStrategy = (title: string) => {
    toast({
      title: "Strategy Started",
      description: `You have started the "${title}" strategy.`,
    });
  };
    ...
    <div className="space-y-8">
          ...
          <Card key={idx}>
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between mb-2 md:mb-4">
                <div className="p-1.5 md:p-2 bg-primary/10 rounded-md">
                  <metric.icon className="h-3 w-3 md:h-4 md:w-4 text-primary" />
                </div>
              </div>
              <div className="text-2xl md:text-3xl font-bold font-serif mb-1">{metric.value}</div>
        ...
        <CardContent className="p-4 md:p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-serif font-semibold text-base md:text-lg mb-2">AI Strategic Analysis</h3>
              <p className="text-xs md:text-sm text-muted-foreground mb-4 line-clamp-3 md:line-clamp-none">
              ...
              <Button size="sm" onClick={() => setLocation('/insights')} data-testid="button-view-full-analysis" className="w-full md:w-auto">
                View Full Analysis
      ...
      <Tabs defaultValue="recommended" className="w-full">
        ...
        <TabsContent value="recommended" className="mt-6">
          <div className="flex overflow-x-auto snap-x no-scrollbar gap-4 pb-4 md:block md:space-y-6 md:pb-0">
            {strategies
              .filter((s) => s.status === "recommended")
              .map((strategy, idx) => (
                <div key={idx} className="min-w-[90%] sm:min-w-[400px] snap-center md:min-w-0">
                  <Card className="hover-elevate active-elevate-2 transition-all h-full" data-testid={`card-strategy-${idx}`}>
                    <CardHeader className="p-4 md:p-6 pb-2 md:pb-6">
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <CardTitle className="font-serif text-lg md:text-xl truncate">{strategy.title}</CardTitle>
                            <Badge
                              variant="outline"
                              className={`shrink-0 text-[10px] md:text-xs px-1.5 md:px-2.5 ${strategy.priority === "high"
                                  ? "bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20"
                                  : "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20"
                                }`}
                            >
                              {strategy.priority === "high" ? "High Priority" : "Medium"}
                            </Badge>
                          </div>
                          <CardDescription className="line-clamp-2 md:line-clamp-none">{strategy.description}</CardDescription>
                        </div>
                        <div className="flex gap-2 w-full md:w-auto mt-2 md:mt-0">
                          <Button variant="outline" size="sm" className="flex-1 md:flex-none text-xs" onClick={() => handleLearnMore(strategy.title)} data-testid="button-learn-more">
                            Learn More
                          </Button>
                          <Button size="sm" className="flex-1 md:flex-none text-xs" onClick={() => handleStartStrategy(strategy.title)} data-testid="button-start-strategy">
                            Start
                          </Button>
                        </div>
                      </div>
                    ...
                    <CardContent className="space-y-4 md:space-y-6 p-4 md:p-6 pt-0 md:pt-0">
                      <div className="grid grid-cols-3 gap-2 md:gap-4 text-xs md:text-sm">
                        <div>
                          ...
                          <p className="font-medium truncate">{strategy.timeline}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground mb-1">ROI</p>
                          <p className="font-medium text-green-600 dark:text-green-400 truncate">{strategy.roi}</p>
                        </div>
                        <div>
                      ...
                      <div className="space-y-2 md:space-y-3">
                        ...
                        <Progress value={strategy.impact} className="h-1.5 md:h-2" />
                      </div>
            ...
            .map((strategy, idx) => (
              <Card key={idx} className="border-primary/20">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button className="w-full" variant="outline" onClick={() => toast({ title: "Progress Details", description: "Showing detailed progress..." })} data-testid="button-view-progress">
        ...
        <TabsContent value="completed" className="mt-6">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <Button onClick={() => setLocation('/learning-paths')} data-testid="button-explore-strategies">Explore Strategies</Button>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
