...
export const MagicCard = React.forwardRef<HTMLDivElement, MagicCardProps>(({
  ...
  const initializeParticles = useCallback(() => {
    if (particlesInitialized.current || !internalRef.current) return;
  ...
  const animateParticles = useCallback(() => {
    if (!internalRef.current || !isHoveredRef.current || !enableParticles) return;
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = setTimeout(() => {
        ...
        internalRef.current!.appendChild(clone);
